...
public class CityTrie {
    ...
    /**
     * Searches the trie according to the city name, state and country
     * and returns the city, if found.
     *
     * @param name    city name
     * @param state   city state or province
     * @param country city country
     * @return city corresponding to search of null if not found
     */
    public City get(String name, String state, String country) {
        Node node = get(name, 0, root);
        City result = null;

        if (node != null) {
            for (City c : node.elements) {
                if (c.getState().equals(state) && c.getCountry().equals(country)) {
                    return c;
                }
            }
        }
     ...
     *
     * @param key     key to be added in the trie
